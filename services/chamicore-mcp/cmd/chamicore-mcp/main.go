...
func main() {
	...
	case config.TransportStdio:
		if runErr := server.RunStdio(ctx, os.Stdin, os.Stdout, registry, version, logger); runErr != nil {
	...
	case config.TransportHTTP:
		httpServer := server.NewHTTPServer(cfg, version, commit, buildDate, api.ToolsContract, registry, logger)
		srv := &http.Server{
			Addr:              cfg.ListenAddr,
